...
import com.example.tcrsuperapp.view.staff.sp.ActivitySp

class AdapterSpCust(dataList: ArrayList<Customer>?): RecyclerView.Adapter<AdapterSpCust.CustViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CustViewHolder, @SuppressLint("RecyclerView") position: Int) {
        ...
        holder.viewHolder.setOnClickListener { v ->
            SP = v.context.getSharedPreferences("SP", Context.MODE_PRIVATE)
            val intent = Intent(v.context, ActivitySp::class.java)
            val editor = SP.edit()
