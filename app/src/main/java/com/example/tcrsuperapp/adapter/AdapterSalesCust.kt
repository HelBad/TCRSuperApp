...
class AdapterSalesCust(dataList: ArrayList<Customer>?): RecyclerView.Adapter<AdapterSalesCust.CustViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CustViewHolder, @SuppressLint("RecyclerView") position: Int) {
        ...
        holder.viewHolder.setOnClickListener { v ->
            ...
            SP1 = v.context.getSharedPreferences("Sales", Context.MODE_PRIVATE)
            if(SP.getString("level", "").toString() == "Admin") {
                val intent = Intent(v.context,
                    com.example.tcrsuperapp.view.admin.sales.ActivitySalesAwal::class.java)
                val editor = SP1.edit()
                ...
                val intent = Intent(v.context,
                    com.example.tcrsuperapp.view.sales.sales.ActivitySalesAwal::class.java)
                val editor = SP1.edit()
                editor.putString("kode", dataList[position].kode)
                editor.putString("perusahaan", dataList[position].perusahaan)
                editor.apply()
